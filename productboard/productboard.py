...
class ListEntitiesAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"entities": data.get("data", []), "next_cursor": next_cursor, "result": True}, cost_usd=0.0
            )

        except Exception as e:
            return ActionResult(data={"entities": [], "result": False, "error": str(e)}, cost_usd=0.0)
...
class GetEntityAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(data={"entity": {}, "result": False, "error": str(e)}, cost_usd=0.0)
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            if custom_fields and not isinstance(custom_fields, dict):
                return ActionResult(
                    data={"entity": {}, "result": False, "error": "custom_fields must be an object"}, cost_usd=0.0
                )

            entity_data = {"type": inputs["type"], "fields": {"name": inputs["name"]}}

            if inputs.get("parent_id"):
                entity_data["relationships"] = {"parent": {"data": {"id": inputs["parent_id"]}}}
...
class UpdateEntityAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            if inputs.get("name"):
                patch_operations.append({"op": "set", "field": "name", "value": inputs["name"]})
            if inputs.get("status"):
                patch_operations.append({"op": "set", "field": "status", "value": build_status_value(inputs["status"])})
            if inputs.get("description"):
                patch_operations.append({"op": "set", "field": "description", "value": inputs["description"]})
            if inputs.get("archived") is not None:
                patch_operations.append({"op": "set", "field": "archived", "value": inputs["archived"]})
            if custom_fields:
                for field_name, field_value in custom_fields.items():
                    patch_operations.append({"op": "set", "field": field_name, "value": field_value})

            if not patch_operations:
                return ActionResult(
                    data={"entity": {}, "result": False, "error": "No fields provided to update"}, cost_usd=0.0
                )
...
class GetEntityConfigurationAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(data={"configuration": {}, "result": False, "error": str(e)}, cost_usd=0.0)
...
class ListNotesAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"notes": data.get("data", []), "next_cursor": next_cursor, "result": True}, cost_usd=0.0
            )
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            if tags and not isinstance(tags, list):
                return ActionResult(data={"note": {}, "result": False, "error": "tags must be an array"}, cost_usd=0.0)

            messages = inputs.get("messages")
            if messages and not isinstance(messages, list):
                return ActionResult(
                    data={"note": {}, "result": False, "error": "messages must be an array"}, cost_usd=0.0
                )

            note_data = {"type": inputs["type"], "fields": {"name": inputs["name"]}}
            ...
            if inputs.get("customer_email"):
                relationships["customer"] = {"data": {"type": "user", "email": inputs["customer_email"]}}
            if inputs.get("company_id"):
                relationships["customer"] = {"data": {"type": "company", "id": inputs["company_id"]}}
            if relationships:
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            if tags_to_add and not isinstance(tags_to_add, list):
                return ActionResult(
                    data={"note": {}, "result": False, "error": "tags_to_add must be an array"}, cost_usd=0.0
                )

            tags_to_remove = inputs.get("tags_to_remove")
            if tags_to_remove and not isinstance(tags_to_remove, list):
                return ActionResult(
                    data={"note": {}, "result": False, "error": "tags_to_remove must be an array"}, cost_usd=0.0
                )
            ...
            if inputs.get("content"):
                patch_operations.append({"op": "set", "field": "content", "value": inputs["content"]})
            if inputs.get("owner_email"):
                patch_operations.append({"op": "set", "field": "owner", "value": {"email": inputs["owner_email"]}})
            if inputs.get("archived") is not None:
                patch_operations.append({"op": "set", "field": "archived", "value": inputs["archived"]})
            if inputs.get("processed") is not None:
                patch_operations.append({"op": "set", "field": "processed", "value": inputs["processed"]})
            if tags_to_add:
                patch_operations.append({"op": "addItems", "field": "tags", "value": tags_to_add})
            if tags_to_remove:
                patch_operations.append({"op": "removeItems", "field": "tags", "value": tags_to_remove})

            if not patch_operations:
                return ActionResult(
                    data={"note": {}, "result": False, "error": "No fields provided to update"}, cost_usd=0.0
                )
...
class ListAnalyticsReportsAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"reports": data.get("data", []), "next_cursor": next_cursor, "result": True}, cost_usd=0.0
            )
