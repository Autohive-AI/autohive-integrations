...
TEST_API_TOKEN = "your-circle-api-token-here"  # nosec B105  # Replace with actual token
...
async def test_get_community_info():
    ...
    """
    auth = {"credentials": {"api_token": TEST_API_TOKEN}}
    ...
    async with ExecutionContext(auth=auth) as context:
        try:
            ...
            if result.result.data.get("result"):
                ...
                print("V Success: Retrieved community info")
    ...
    async with ExecutionContext(auth=auth) as context:
        try:
            ...
            if result.result.data.get("result"):
                ...
                print("V Success: Retrieved space details")
...
async def test_search_posts():
    ...
    inputs = {"status": "published", "per_page": 10}
    ...
    async with ExecutionContext(auth=auth) as context:
        try:
            ...
            if result.result.data.get("result"):
                ...
                print("V Success: Retrieved post details")
...
async def test_create_post_with_markdown():
    ...
    auth = {"credentials": {"api_token": "your-circle-api-token-here"}}  # nosec B105
...
async def test_get_member():
    ...
    async with ExecutionContext(auth=auth) as context:
        try:
            ...
            if result.result.data.get("result"):
                member = result.result.data.get("member", {})
                print("V Success: Retrieved member details")
                print(f"  Name: {member.get('name')}")
                print(f"  Email: {member.get('email')}")
...
async def test_search_events():
    ...
    inputs = {"time_filter": "upcoming", "per_page": 10}
    ...
    async with ExecutionContext(auth=auth) as context:
        try:
            ...
            if result.result.data.get("result"):
                ...
                print("V Success: Retrieved event details")
                name = event.get("name", "Unknown")
                safe_name = name.encode("ascii", "ignore").decode("ascii")
...
async def test_create_comment():
    ...
    inputs = {"post_id": str(post_id), "body": "This is a test comment from the Autohive Circle integration!"}
    ...
    async with ExecutionContext(auth=auth) as context:
        try:
            ...
            if result.data.get("result"):
                ...
                print("V Success: Created comment")
...
async def test_get_post_comments():
    ...
    inputs = {"post_id": str(post_id), "per_page": 20}
