...
class TestPowerBIIntegration(unittest.TestCase):
    ...
    async def test_list_workspaces_success(self):
        ...
        """
        self.mock_context.fetch.return_value = FetchResponse(
            status=200,
            headers={},
            data={
                "value": [
    ...
    async def test_list_workspaces_with_filter(self):
        ...
        """
        self.mock_context.fetch.return_value = FetchResponse(status=200, headers={}, data={"value": []})
    ...
    async def test_refresh_dataset_success(self):
        ...
        """
        self.mock_context.fetch.return_value = FetchResponse(status=202, headers={}, data=None)
    ...
    async def test_get_export_status_success(self):
        ...
        self.mock_context.fetch.return_value = FetchResponse(
            status=200,
            headers={},
            data={"status": "Succeeded", "percentComplete": 100},
        )
    ...
    async def test_refresh_report_success(self):
        ...
        self.mock_context.fetch.side_effect = [
            FetchResponse(
                status=200, headers={}, data={"id": "report1", "name": "Sales Report", "datasetId": "dataset1"}
            ),
    ...
    async def test_refresh_report_no_dataset(self):
        ...
        self.mock_context.fetch.return_value = FetchResponse(
            status=200,
            headers={},
            data={"id": "report1", "name": "Sales Report"},
        )
        ...
        self.assertIsInstance(result, ActionError)
        self.assertIn("dataset", result.message.lower())
