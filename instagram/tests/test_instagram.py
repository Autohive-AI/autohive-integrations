    ...
    async def fetch(
        ...
        **kwargs,
        ...
        if (
            ...
            and "/tags" not in url
            and "/conversations" not in url
            and "/messages" not in url
            and method == "GET"
        ):
            if "biography" in fields or "followers_count" in fields:
                return self._responses.get("GET /me/full", {"id": "17841400000000000", "username": "testbusiness"})
            return self._responses.get("GET /me", {"id": "17841400000000000", "username": "testbusiness"})
...
async def test_get_posts_single():
    ...
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("get_posts", {"media_id": "17841400000000001"}, context)
    data = result.result.data
...
async def test_get_posts_empty():
    ...
    responses = {"GET /me": {"id": "17841400000000000", "username": "testbusiness"}, "GET /media": {"data": []}}
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("get_posts", {}, context)
    data = result.result.data
...
async def test_create_post_image():
    ...
    context = MockExecutionContext(responses)
    result = await instagram.execute_action(
        "create_post",
        {"media_type": "IMAGE", "media_url": "https://example.com/image.jpg", "caption": "My new post!"},
        context,
    )
    data = result.result.data
...
async def test_create_post_reels():
    ...
    result = await instagram.execute_action(
        "create_post",
        {"media_type": "REELS", "media_url": "https://example.com/video.mp4", "caption": "My new reel!"},
        context,
    )
    data = result.result.data
...
async def test_create_post_missing_url():
    ...
    responses = {"GET /me": {"id": "17841400000000000", "username": "testbusiness"}}
    context = MockExecutionContext(responses)

    try:
        await instagram.execute_action("create_post", {"media_type": "IMAGE", "caption": "Missing URL"}, context)
...
async def test_create_post_carousel_too_few():
    ...
    try:
        await instagram.execute_action(
            "create_post", {"media_type": "CAROUSEL", "children": ["https://example.com/image1.jpg"]}, context
        )
        assert False, "Should have raised exception"
    except Exception as e:
...
async def test_create_story_image():
    ...
    context = MockExecutionContext(responses)
    result = await instagram.execute_action(
        "create_story", {"media_type": "IMAGE", "media_url": "https://example.com/story.jpg"}, context
    )
    data = result.result.data
...
async def test_get_comments_success():
    ...
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("get_comments", {"media_id": "17841400000000001"}, context)
    data = result.result.data
...
async def test_manage_comment_reply():
    ...
    responses = {"POST /replies": {"id": "reply_new_123"}}
    context = MockExecutionContext(responses)
    result = await instagram.execute_action(
        "manage_comment",
        {"comment_id": "comment_1", "action": "reply", "message": "Thank you for your feedback!"},
        context,
    )
    data = result.result.data
...
async def test_manage_comment_hide():
    ...
    responses = {"POST /hide": {"success": True}}
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("manage_comment", {"comment_id": "comment_1", "action": "hide"}, context)
    data = result.result.data
...
async def test_manage_comment_unhide():
    ...
    responses = {"POST /hide": {"success": True}}
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("manage_comment", {"comment_id": "comment_1", "action": "unhide"}, context)
    data = result.result.data
...
async def test_manage_comment_reply_missing_message():
    ...
    try:
        await instagram.execute_action("manage_comment", {"comment_id": "comment_1", "action": "reply"}, context)
        assert False, "Should have raised exception"
    except Exception as e:
...
async def test_manage_comment_invalid_action():
    ...
    try:
        await instagram.execute_action(
            "manage_comment", {"comment_id": "comment_1", "action": "invalid_action"}, context
        )
        assert False, "Should have raised exception"
    except Exception as e:
...
async def test_delete_comment_success():
    ...
    responses = {"DELETE": {"success": True}}
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("delete_comment", {"comment_id": "comment_1"}, context)
    data = result.result.data
...
async def test_get_insights_account():
    ...
    context = MockExecutionContext(responses)
    result = await instagram.execute_action("get_insights", {"target_type": "account"}, context)
    data = result.result.data
...
async def test_get_insights_media():
    ...
    context = MockExecutionContext(responses)
    result = await instagram.execute_action(
        "get_insights", {"target_type": "media", "target_id": "17841400000000001"}, context
    )
    data = result.result.data
...
async def test_get_insights_media_missing_target_id():
    ...
    try:
        await instagram.execute_action("get_insights", {"target_type": "media"}, context)
        assert False, "Should have raised exception"
    except Exception as e:
