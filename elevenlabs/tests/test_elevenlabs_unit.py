...
class TestListVoices:
    async def test_returns_voices(self, ctx):
        ctx.fetch.return_value = FetchResponse(
            status=200, headers={}, data={"voices": [{"voice_id": "v1", "name": "Alice"}]}
        )
...
class TestGetVoice:
    async def test_returns_voice(self, ctx):
        ctx.fetch.return_value = FetchResponse(
            status=200, headers={}, data={"voice_id": "v1", "name": "Alice", "category": "premade"}
        )
        inputs = {"voice_id": "v1"}
...
class TestGetVoiceSettings:
    async def test_returns_settings(self, ctx):
        ctx.fetch.return_value = FetchResponse(
            status=200,
            headers={},
            data={"stability": 0.5, "similarity_boost": 0.75, "style": 0.0, "use_speaker_boost": True},
        )
