...
class ListSpreadsheets(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            response = {"files": result.get("files", []), "result": True}
            ...
            return ActionResult(
                data={"files": [], "result": False, "error": f"Google Drive API error: {str(e)}"},
                cost_usd=0.0,
            )
...
class GetSpreadsheet(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"spreadsheet": {}, "result": False, "error": f"Google Sheets API error: {str(e)}"},
                cost_usd=0.0,
            )
        except Exception as e:
...
class ListSheets(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"sheets": [], "result": False, "error": f"Google Sheets API error: {str(e)}"},
                cost_usd=0.0,
            )
        except Exception as e:
...
class ReadRange(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            params: Dict[str, Any] = {"spreadsheetId": spreadsheet_id, "range": a1}
            if value_render:
            ...
            return ActionResult(
                data={"range": result.get("range", a1), "values": result.get("values", []), "result": True},
                cost_usd=0.0,
            )
        except HttpError as e:
            return ActionResult(
            ...
            return ActionResult(
                data={"range": inputs.get("range"), "values": [], "result": False, "error": str(e)},
                cost_usd=0.0,
            )
...
class AppendRows(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"updates": {}, "result": False, "error": f"Google Sheets API error: {str(e)}"},
                cost_usd=0.0,
            )
        except Exception as e:
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            requests = [
                {
                    "repeatCell": {
                        "range": {"sheetId": sheet_id, **grid_range},
            ...
            return ActionResult(
                data={"replies": [], "result": False, "error": f"Google Sheets API error: {str(e)}"},
                cost_usd=0.0,
            )
        except Exception as e:
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            if not isinstance(requests, list) or not all(isinstance(r, dict) for r in requests):
                return ActionResult(
                    data={"result": False, "error": "requests must be an array of objects"},
                    cost_usd=0.0,
                )
            ...
            return ActionResult(
                data={"replies": result.get("replies", []), "dryRun": False, "result": True},
                cost_usd=0.0,
            )
        except HttpError as e:
            return ActionResult(
...
class DuplicateSpreadsheet(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            return ActionResult(
                data={"file_metadata": {}, "result": False, "error": f"Google Drive API error: {str(e)}"},
                cost_usd=0.0,
            )
        except Exception as e:
