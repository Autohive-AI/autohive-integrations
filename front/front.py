...
class CreateMessageAction(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        try:
            ...
            message_data = {"body": inputs["body"], "to": inputs["to"]}
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            if "assignee_id" in inputs:
                update_data["assignee_id"] = inputs.get("assignee_id")
            inbox_id = inputs.get("inbox_id")
            if inbox_id:
                update_data["inbox_id"] = inbox_id
            status = inputs.get("status")
            if status:
                update_data["status"] = status
            status_id = inputs.get("status_id")
            if status_id:
                update_data["status_id"] = status_id
            tags = inputs.get("tags")
            if tags:
                update_data["tag_ids"] = tags
...
class FindConversationAction(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        try:
            ...
            list_result = await list_action.execute({"inbox_id": inbox_id, "limit": limit}, context)
            ...
            for conversation in conversations:
                ...
                # Check if search query matches any field
                if (
                    search_query in subject
                    or search_query in recipient_handle
                    or search_query in recipient_name
                    or search_query in recipient_str
                ):
