...
class TestGetAgedPayables:
    ...
    async def test_happy_path(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            result = await xero.execute_action(
                "get_aged_payables", {"tenant_id": "t-001", "contact_id": "c-001"}, mock_context
            )
    ...
    async def test_calls_correct_endpoint(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action("get_aged_payables", {"tenant_id": "t-001", "contact_id": "c-001"}, mock_context)
...
class TestGetAgedReceivables:
    ...
    async def test_happy_path(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            result = await xero.execute_action(
                "get_aged_receivables", {"tenant_id": "t-001", "contact_id": "c-001"}, mock_context
            )
...
class TestGetBalanceSheet:
    ...
    async def test_calls_balance_sheet_endpoint(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action("get_balance_sheet", {"tenant_id": "t-001"}, mock_context)
    ...
    async def test_optional_periods_param(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action("get_balance_sheet", {"tenant_id": "t-001", "periods": 3}, mock_context)
...
class TestGetProfitAndLoss:
    ...
    async def test_calls_profit_loss_endpoint(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action("get_profit_and_loss", {"tenant_id": "t-001"}, mock_context)
    ...
    async def test_optional_date_range_params(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action(
                "get_profit_and_loss",
                {"tenant_id": "t-001", "from_date": "2024-01-01", "to_date": "2024-12-31"},
                mock_context,
            )
...
class TestGetTrialBalance:
    ...
    async def test_payments_only_param(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action("get_trial_balance", {"tenant_id": "t-001", "payments_only": True}, mock_context)
    ...
    async def test_happy_path(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            mock_limiter.make_request = AsyncMock(
                return_value={"Accounts": [{"AccountID": "a-001", "Code": "200", "Name": "Sales"}]}
            )
...
class TestGetPayments:
    ...
    async def test_happy_path(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            mock_limiter.make_request = AsyncMock(return_value={"Payments": [{"PaymentID": "p-001", "Amount": 500.0}]})
    ...
    async def test_pagination_params(self, mock_context):
        with patch.object(_mod, "rate_limiter") as mock_limiter:
            ...
            await xero.execute_action("get_payments", {"tenant_id": "t-001", "page": 2, "pageSize": 50}, mock_context)
