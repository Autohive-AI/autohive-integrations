...
from autohive_integrations_sdk.integration import ActionError, ResultType
...
async def first_contact_id(live_context, tenant_id):
    ...
    result = await xero.execute_action(
        "find_contact_by_name", {"tenant_id": tenant_id, "contact_name": "Test"}, live_context
    )
...
class TestFindContactByName:
    async def test_returns_contacts_list(self, live_context, tenant_id):
        result = await xero.execute_action(
            "find_contact_by_name", {"tenant_id": tenant_id, "contact_name": "a"}, live_context
        )
...
class TestGetAttachments:
    async def test_returns_attachments_for_invoice(self, live_context, tenant_id, first_invoice_id):
        inputs = {"tenant_id": tenant_id, "endpoint": "Invoices", "guid": first_invoice_id}
...
class TestCreateUpdateDeletePurchaseOrder:
    ...
    async def test_full_purchase_order_lifecycle(self, live_context, tenant_id, first_contact_id):
        ...
        line_items = [{"Description": "Integration test item", "Quantity": 1, "UnitAmount": 10.00, "AccountCode": "200"}]

        # Create
        create_result = await xero.execute_action(
            "create_purchase_order",
            {"tenant_id": tenant_id, "contact": contact, "line_items": line_items, "status": "DRAFT"},
            live_context,
        )
        ...
        update_result = await xero.execute_action(
            ...
            {"tenant_id": tenant_id, "purchase_order_id": po_id, "reference": "integration-test"},
            live_context,
        )
        ...
        note_result = await xero.execute_action(
            ...
            {"tenant_id": tenant_id, "purchase_order_id": po_id, "note": "Integration test note"},
            live_context,
        )
...
class TestCreateUpdateSalesInvoice:
    ...
    async def test_create_and_update_draft_invoice(self, live_context, tenant_id, first_contact_id):
        contact = {"ContactID": first_contact_id}
        line_items = [{"Description": "Integration test service", "Quantity": 1, "UnitAmount": 50.00, "AccountCode": "200"}]
        ...
        update_result = await xero.execute_action(
            ...
            {"tenant_id": tenant_id, "invoice_id": invoice_id, "reference": "integration-test"},
            live_context,
        )
...
class TestCreateUpdatePurchaseBill:
    ...
    async def test_create_and_update_draft_bill(self, live_context, tenant_id, first_contact_id):
        contact = {"ContactID": first_contact_id}
        line_items = [{"Description": "Integration test expense", "Quantity": 1, "UnitAmount": 25.00, "AccountCode": "300"}]
        ...
        update_result = await xero.execute_action(
            ...
            {"tenant_id": tenant_id, "invoice_id": bill_id, "reference": "integration-test"},
            live_context,
        )
        assert assert_action_success(update_result).get("Invoices")
