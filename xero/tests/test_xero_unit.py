...
def mock_context():
    ...
    ctx.auth = {"credentials": {"access_token": "test_token", "tenant_id": "test_tenant"}}  # nosec B105
    return ctx
            ...
            mock_limiter.make_request = AsyncMock(
                return_value={
                    "Accounts": [
                        ...
                        {"AccountID": "a2", "Code": "400", "Name": "Advertising", "Type": "EXPENSE"},
                    ]
                }
            )
    ...
    async def test_returns_payments(self, mock_context):
        with patch("xero_mod.rate_limiter") as mock_limiter:
            mock_limiter.make_request = AsyncMock(
                return_value={
                    "Payments": [
                        {"PaymentID": "pay1", "Amount": 500.0, "PaymentType": "ACCRECPAYMENT", "Status": "AUTHORISED"}
                    ]
                }
            )
            inputs = {"tenant_id": "test_tenant"}
    ...
    async def test_returns_transactions(self, mock_context):
        with patch("xero_mod.rate_limiter") as mock_limiter:
            mock_limiter.make_request = AsyncMock(
                return_value={
                    "BankTransactions": [
                        {"BankTransactionID": "bt1", "Type": "SPEND", "Total": 250.0, "Status": "AUTHORISED"}
                    ]
                }
            )
            inputs = {"tenant_id": "test_tenant"}
...
class TestGetAgedPayables:
    async def test_returns_report(self, mock_context):
        with patch("xero_mod.rate_limiter") as mock_limiter:
            mock_limiter.make_request = AsyncMock(
                return_value={"Reports": [{"ReportID": "AgedPayablesByContact", "ReportName": "Aged Payables"}]}
            )
    ...
    async def test_creates_invoice(self, mock_context):
        with patch("xero_mod.rate_limiter") as mock_limiter:
            ...
            inputs = {
                ...
                "line_items": [{"Description": "Consulting", "Quantity": 5, "UnitAmount": 100.0, "AccountCode": "200"}],
            }
    ...
    async def test_returns_purchase_orders(self, mock_context):
        with patch("xero_mod.rate_limiter") as mock_limiter:
            mock_limiter.make_request = AsyncMock(
                return_value={
                    "PurchaseOrders": [
                        {"PurchaseOrderID": "po1", "PurchaseOrderNumber": "PO-001", "Status": "AUTHORISED"}
                    ]
                }
            )
            inputs = {"tenant_id": "test_tenant"}
    ...
    async def test_creates_purchase_order(self, mock_context):
        with patch("xero_mod.rate_limiter") as mock_limiter:
            ...
            inputs = {
                ...
                "contact": {"ContactID": "c1"},
                "line_items": [
                    {"Description": "Office Supplies", "Quantity": 5, "UnitAmount": 50.0, "AccountCode": "200"}
                ],
            }
