...
import asyncio
import context as _ctx  # noqa: F401 - imported for side effect (adds parent to sys.path)
from bigquery import bigquery
from autohive_integrations_sdk import ExecutionContext

# Test configuration - update these values
TEST_PROJECT_ID = "your-project-id"
TEST_ACCESS_TOKEN = "your-oauth-access-token"  # nosec B105


def get_test_context():
    """Create a test execution context with OAuth credentials."""
    return ExecutionContext(auth={"auth_type": "PlatformOauth2", "credentials": {"access_token": TEST_ACCESS_TOKEN}})
...
async def test_list_projects():
    ...
    async with get_test_context() as context:
        result = await bigquery.execute_action("list_projects", {"max_results": 10}, context)
...
async def test_list_datasets():
    ...
    async with get_test_context() as context:
        result = await bigquery.execute_action("list_datasets", {"project_id": TEST_PROJECT_ID}, context)
        print(f"Result: {result.get('result')}")
    ...
    async with get_test_context() as context:
        ...
        if result.get("result"):
            ...
            get_result = await bigquery.execute_action(
                "get_dataset", {"project_id": TEST_PROJECT_ID, "dataset_id": test_dataset_id}, context
            )
        ...
        delete_result = await bigquery.execute_action(
            "delete_dataset", {"project_id": TEST_PROJECT_ID, "dataset_id": test_dataset_id}, context
        )
...
async def test_list_tables():
    ...
    async with get_test_context() as context:
        # First get a dataset
        datasets_result = await bigquery.execute_action("list_datasets", {"project_id": TEST_PROJECT_ID}, context)

        if datasets_result.get("datasets"):
            dataset_id = datasets_result["datasets"][0]["dataset_id"]
            result = await bigquery.execute_action(
                "list_tables", {"project_id": TEST_PROJECT_ID, "dataset_id": dataset_id}, context
            )
            print(f"Result: {result.get('result')}")
    ...
    async with get_test_context() as context:
        ...
        for ds in datasets_result.get("datasets", []):
            tables_result = await bigquery.execute_action(
                "list_tables", {"project_id": TEST_PROJECT_ID, "dataset_id": ds["dataset_id"]}, context
            )
            ...
            if tables_result.get("tables"):
                ...
                result = await bigquery.execute_action(
                    "get_table",
                    {"project_id": TEST_PROJECT_ID, "dataset_id": ds["dataset_id"], "table_id": table["table_id"]},
                    context,
                )
                print(f"Result: {result.get('result')}")
...
async def test_list_jobs():
    ...
    print("\n=== Testing list_jobs ===")
    async with get_test_context() as context:
        result = await bigquery.execute_action("list_jobs", {"project_id": TEST_PROJECT_ID, "max_results": 10}, context)
        print(f"Result: {result.get('result')}")
