...
def test_placeholder_detection():
    ...
    runner.test("Detects simple bracket placeholder", placeholders == ["[Company Name]"])

    # Test 2: Curly braces
    placeholders, _ = detect_placeholders_with_metadata("{Company}")
    runner.test("Detects curly brace placeholder", placeholders == ["{Company}"])

    # Test 3: Double curly
    placeholders, _ = detect_placeholders_with_metadata("{{Date}}")
    runner.test("Detects double curly placeholder", placeholders == ["{{Date}}"])

    # Test 4: Multiple placeholders
    placeholders, _ = detect_placeholders_with_metadata("[A] and [B] and [C]")
    runner.test("Detects multiple placeholders", len(placeholders) == 3 and "[A]" in placeholders)

    # Test 5: Special characters
    placeholders, _ = detect_placeholders_with_metadata("[$x,xxx]")
    runner.test("Handles special characters (dollar, comma)", placeholders == ["[$x,xxx]"])

    # Test 6: No placeholders
    placeholders, _ = detect_placeholders_with_metadata("Plain text with no placeholders")
    runner.test("Returns empty for plain text", placeholders == [])
...
def test_metadata_parsing():
    ...
    runner.test("Extracts Fontsize=32pt", meta.get("fontsize") == "32pt")
    ...
    runner.test("Extracts Color=#FF0000", meta.get("color") == "#FF0000")
    ...
    runner.test("Extracts Font=Sofia Pro", meta.get("font") == "Sofia Pro")
    ...
    runner.test(
        ...
        and meta.get("bold") == "true"
        and meta.get("italic") == "false"
        and meta.get("color") == "#003366",
    )
    ...
    runner.test(
        "Case-insensitive parsing (BOLD, FONTSIZE)", meta.get("bold") == "true" and meta.get("fontsize") == "24PT"
    )

    # Test 12: No metadata (simple placeholder)
    _, metadata = detect_placeholders_with_metadata("[Company]")
    runner.test("Simple placeholder has no metadata", metadata == {})
...
def test_font_path_resolution():
    ...
    runner.test("Resolves Calibri Bold variant", path is not None and "Bold" in path if path else False)

    # Test case insensitivity
    path = get_font_path("CALIBRI")
    runner.test("Case-insensitive font resolution (CALIBRI)", path is not None and "Calibri" in path if path else False)
    ...
    runner.test("Returns None for nonexistent font", path is None)
...
def test_markdown_detection():
    ...
    for text, expected in test_cases:
        ...
        runner.test(f"Detects markdown in '{text[:30]}': {expected}", result == expected)
...
def test_font_size_calculation():
    ...
    runner.test("Short text stays at max size (24pt)", size == 24, f"Got: {size}pt")
    ...
    runner.test("Long text scales down from 18pt", size < 18, f"Scaled to: {size}pt")
    ...
    runner.test("Minimum size is 10pt", size >= 10, f"Got: {size}pt")

    # Test 4: Empty text returns max size
    size = calculate_best_fit_font_size("", width_inches=5.0, height_inches=2.0, max_font_size=20)
    runner.test("Empty text returns max size", size == 20)

    # Test 5: Bullet text gets extra spacing
    bullet_text = "• Point 1\n• Point 2\n• Point 3"
    size_no_bullets = calculate_best_fit_font_size(
        bullet_text, width_inches=4.0, height_inches=2.0, max_font_size=18, is_bullets=False
    )
    size_with_bullets = calculate_best_fit_font_size(
        bullet_text, width_inches=4.0, height_inches=2.0, max_font_size=18, is_bullets=True
    )
    runner.test(
...
def test_edge_cases():
    ...
    runner.test("[$x,xxx] detected as placeholder", placeholders == ["[$x,xxx]"])
    runner.test("[$x,xxx] has no metadata", metadata == {})

    # Test 2: Email-like placeholder
    placeholders, _ = detect_placeholders_with_metadata("[email@example.com]")
    runner.test("Email-like placeholder detected", placeholders == ["[email@example.com]"])

    # Test 3: Date with slashes
    placeholders, _ = detect_placeholders_with_metadata("[Date / 13 October 2025]")
    runner.test("Date with slashes detected", placeholders == ["[Date / 13 October 2025]"])

    # Test 4: Mixed bracket types
    placeholders, _ = detect_placeholders_with_metadata("[Bracket] and {Curly}")
    runner.test("Detects mixed bracket types", len(placeholders) == 2)
    ...
    runner.test("Handles very long placeholder", len(placeholders) == 1)

    # Test 7: Unicode characters
    placeholders, _ = detect_placeholders_with_metadata("[Café]")
    runner.test("Handles unicode in placeholder", placeholders == ["[Café]"])
    ...
    runner.test(
        "Last metadata value wins (!Bold overrides Bold)", meta.get("bold") == "false", f"Got: {meta.get('bold')}"
    )
...
def test_metadata_formats():
    ...
    for placeholder, expected_value in fontsize_tests:
        ...
        runner.test(f"Fontsize format: {placeholder}", meta.get("fontsize") == expected_value)
    ...
    for (p1, v1), (p2, v2) in test_pairs:
        ...
        runner.test(f"Shorthand {p1} equivalent to {p2}", m1.get(key) == v1 and m2.get(key) == v2)
    ...
    for placeholder, key, expected_value in case_tests:
        _, metadata = detect_placeholders_with_metadata(placeholder)
        meta = metadata.get(placeholder, {})
        runner.test(f"Case-insensitive: {placeholder}", meta.get(key) == expected_value)
...
def test_combined_functionality():
    ...
    runner.test("Real SOW placeholder detected", len(placeholders) == 1)
    ...
    runner.test("Detects all placeholders in sentence", len(placeholders) == 3)
    ...
    runner.test("Mixed metadata and plain placeholders", len(placeholders) == 3 and len(metadata) == 1)
    ...
    for p in agent_placeholders:
        ...
        meta = metadata.get(p, {})
        runner.test(f"Agent format: {p}", len(placeholders) == 1 and len(meta) > 1)
...
def test_font_availability():
    ...
    runner.test("fonts/ directory exists", os.path.exists(fonts_dir))

    # Check key font files exist
    expected_fonts = ["Calibri.ttf", "Calibri-Bold.ttf", "Arial.ttf", "Times.ttf", "Courier.ttf"]

    for font_file in expected_fonts:
        font_path = os.path.join(fonts_dir, font_file)
        runner.test(f"Font file exists: {font_file}", os.path.exists(font_path))

    # Check config files exist
    config_files = ["font_alternatives.json", "google_fonts_downloader.py", "README.md"]

    for config_file in config_files:
        config_path = os.path.join(fonts_dir, config_file)
        runner.test(f"Config file exists: {config_file}", os.path.exists(config_path))
