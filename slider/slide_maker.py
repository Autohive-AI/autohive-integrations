...
def load_presentation_from_files(presentation_id: str, files: List[Dict[str, Any]]) -> None:
    ...
    if presentation_id not in presentations and files:
        ...
        raise ValueError(
            f"No valid PowerPoint file found in files. Tried to load: {available_files}. Files may be corrupted or not PowerPoint format."
        )
...
async def save_and_return_presentation(
    original_result: Dict[str, Any], presentation_id: str, context: ExecutionContext, custom_filename: str = None
) -> Dict[str, Any]:
...
def validate_and_adjust_position(requested_pos, existing_elements, slide_dims, auto_position=False):
    ...
    if has_overlap:
        if not auto_position:
            return {
                "fits": False,
                "reason": "Element overlaps with existing element(s). Enable auto_position to find alternative placement.",
            }
...
def calculate_best_fit_font_size_heuristic(
    text, width_inches, height_inches, max_font_size=18, has_formatting=True, is_bullets=False, debug=False
):
    """
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        if slide_index >= len(prs.slides):
            if len(prs.slides) == 0:
                ...
                raise ValueError(
                    f"Slide index {slide_index} out of range. Valid range: 0-{len(prs.slides) - 1} ({len(prs.slides)} slides total)."
                )
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        if shape_index >= len(slide.shapes):
            if len(slide.shapes) == 0:
                ...
                raise ValueError(
                    f"Shape index {shape_index} out of range. Valid range: 0-{len(slide.shapes) - 1} ({len(slide.shapes)} elements total)."
                )
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        if element_index >= len(slide.shapes):
            if len(slide.shapes) == 0:
                ...
                raise ValueError(
                    f"Element index {element_index} out of range. Valid range: 0-{len(slide.shapes) - 1} ({len(slide.shapes)} elements total)."
                )
...
class GetElementStylingAction(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        styling_info = (
            f'{element_type.upper()} @({left_inches:.1f},{top_inches:.1f}) {width_inches:.1f}×{height_inches:.1f}":'
        )
    ...
    def _get_cell_font_info(self, cell):
        ...
        try:
            ...
            if (not font_parts) or (
                font_parts and not any("Lato" in part or "Arial" in part or "Calibri" in part for part in font_parts)
            ):
                ...
                if p.runs:
                    for run in p.runs[:2]:  # Check first 2 runs
                        if hasattr(run, "font") and run.font is not None:
                            ...
                            if (
                                hasattr(run_font, "color")
                                and run_font.color
                                and not any(part == "white" or part.startswith("#") for part in font_parts)
                            ):
                                try:
                            ...
                            matches_found.append(
                                {
                                    ...
                                    "content": shape.text_frame.text[:80] + "..."
                    ...
                    try:
                        ...
                        if forced_font_size:
                            ...
                            try:
                                ...
                                has_markdown = has_markdown_formatting(final_text)
                                has_bullets = any(
                                    bullet in final_text for bullet in ["•", "◦", "▪", "▫", "‣", "-", "*"]
                                )
                                best_fit_size = calculate_best_fit_font_size(
                                    final_text,
                                    cell_width,
