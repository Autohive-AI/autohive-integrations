...
def _parse_scheduled_time(scheduled_time: str | int) -> int:
    ...
    if isinstance(scheduled_time, int):
        ...
        raise ValueError(f"scheduled_time must be a string or integer, got {type(scheduled_time).__name__}")
...
class CreatePostAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        if media_type == "photo":
            ...
            data = {"caption": message, "url": media_url, "access_token": page_token}
            if is_scheduled:
            ...
            data = {"description": message, "file_url": media_url, "access_token": page_token}
            if is_scheduled:
                data["published"] = "false"
                data["scheduled_publish_time"] = scheduled_time
...
class DeletePostAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        await context.fetch(f"{GRAPH_API_BASE}/{post_id}", method="DELETE", params={"access_token": page_token})

        return ActionResult(data={"success": True, "deleted_post_id": post_id})
