...
class GetCommentsAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        params = {"fields": fields, "limit": limit, "access_token": page_token, "summary": "true"}
...
class ManageCommentAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        if action == "reply":
            ...
            return ActionResult(data={"success": True, "action_taken": action, "is_hidden": is_hidden})

        elif action == "like":
            await context.fetch(
                f"{GRAPH_API_BASE}/{comment_id}/likes", method="POST", data={"access_token": page_token}
            )
            return ActionResult(data={"success": True, "action_taken": "like"})

        elif action == "unlike":
            await context.fetch(
                f"{GRAPH_API_BASE}/{comment_id}/likes", method="DELETE", params={"access_token": page_token}
            )
...
class DeleteCommentAction(ActionHandler):
    ...
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        ...
        await context.fetch(f"{GRAPH_API_BASE}/{comment_id}", method="DELETE", params={"access_token": page_token})
