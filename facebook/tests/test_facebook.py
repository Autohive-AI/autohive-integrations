...
class MockExecutionContext:
    ...
    async def fetch(
        ...
        **kwargs,
        ...
        self._requests.append({"url": url, "method": method, "params": params, "data": data})
...
async def test_list_pages_success():
    ...
    responses = {
        "GET /me/accounts": {
            "data": [
                {"id": "123456789", "name": "My Business Page", "category": "Business", "followers_count": 5000},
                {"id": "987654321", "name": "My Personal Brand", "category": "Personal Blog", "followers_count": 1200},
            ]
        }
    }
...
async def test_list_pages_empty():
    ...
    responses = {"GET /me/accounts": {"data": []}}
    context = MockExecutionContext(responses)
    result = await facebook.execute_action("list_pages", {}, context)
    data = result.result.data
...
async def test_get_posts_list():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action("get_posts", {"page_id": "123456789"}, context)
    data = result.result.data
...
async def test_get_posts_single():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action("get_posts", {"page_id": "123456789", "post_id": "123456789_111"}, context)
    data = result.result.data
...
async def test_create_post_text():
    ...
    result = await facebook.execute_action(
        "create_post", {"page_id": "123456789", "message": "Hello, this is a test post!"}, context
    )
    data = result.result.data

    assert data["post_id"] == "123456789_111222333"
    assert "facebook.com" in data["permalink_url"]
    assert not data["is_scheduled"]
...
async def test_create_post_scheduled_unix_timestamp():
    ...
    result = await facebook.execute_action(
        "create_post",
        {"page_id": "123456789", "message": "This post will be published later!", "scheduled_time": str(future_time)},
        context,
    )
    data = result.result.data

    assert data["post_id"] == "123456789_777888999"
    assert data["is_scheduled"]
...
async def test_create_post_scheduled_iso_format():
    ...
    result = await facebook.execute_action(
        "create_post",
        {"page_id": "123456789", "message": "Scheduled with ISO format!", "scheduled_time": future_iso},
        context,
    )
    data = result.result.data
...
async def test_create_post_scheduled_too_soon():
    ...
    try:
        await facebook.execute_action(
            "create_post", {"page_id": "123456789", "message": "Too soon!", "scheduled_time": str(too_soon)}, context
        )
...
async def test_create_post_scheduled_too_far():
    ...
    try:
        await facebook.execute_action(
            "create_post",
            {"page_id": "123456789", "message": "Too far in future!", "scheduled_time": str(too_far)},
            context,
        )
        assert False, "Should have raised exception"
    except ValueError as e:
...
async def test_create_post_scheduled_invalid_format():
    ...
    try:
        await facebook.execute_action(
            "create_post",
            {"page_id": "123456789", "message": "Invalid format!", "scheduled_time": "not-a-valid-time"},
            context,
        )
        assert False, "Should have raised exception"
    except ValueError as e:
...
async def test_create_post_page_not_found():
    ...
    try:
        await facebook.execute_action(
            "create_post", {"page_id": "nonexistent_page", "message": "This should fail"}, context
        )
        assert False, "Should have raised exception"
...
async def test_create_post_photo_missing_url():
    ...
    try:
        await facebook.execute_action(
            "create_post", {"page_id": "123456789", "message": "Missing photo URL", "media_type": "photo"}, context
        )
        assert False, "Should have raised exception"
    except Exception as e:
...
async def test_create_post_video_missing_url():
    ...
    try:
        await facebook.execute_action(
            "create_post", {"page_id": "123456789", "message": "Missing video URL", "media_type": "video"}, context
        )
        assert False, "Should have raised exception"
    except Exception as e:
        assert "media_url is required" in str(e)
...
async def test_create_post_link_missing_url():
    ...
    try:
        await facebook.execute_action(
            "create_post", {"page_id": "123456789", "message": "Missing link URL", "media_type": "link"}, context
        )
        assert False, "Should have raised exception"
    except Exception as e:
        assert "media_url is required" in str(e)
...
async def test_delete_post_success():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action("delete_post", {"page_id": "123456789", "post_id": "123456789_111"}, context)
    data = result.result.data
...
async def test_get_comments_success():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action("get_comments", {"post_id": "123456789_111"}, context)
    data = result.result.data
...
async def test_get_comments_include_hidden():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action(
        "get_comments", {"post_id": "123456789_111", "include_hidden": True}, context
    )
    data = result.result.data

    assert len(data["comments"]) == 2
    assert data["comments"][1]["is_hidden"]
...
async def test_manage_comment_hide():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action(
        "manage_comment", {"page_id": "123456789", "comment_id": "123456789_111_c1", "action": "hide"}, context
    )
    data = result.result.data
...
async def test_manage_comment_unhide():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action(
        "manage_comment", {"page_id": "123456789", "comment_id": "123456789_111_c1", "action": "unhide"}, context
    )
    data = result.result.data

    assert data["success"]
    assert data["action_taken"] == "unhide"
    assert not data["is_hidden"]
...
async def test_manage_comment_reply_missing_message():
    ...
    try:
        await facebook.execute_action(
            "manage_comment", {"page_id": "123456789", "comment_id": "123456789_111_c1", "action": "reply"}, context
        )
        assert False, "Should have raised exception"
    except Exception as e:
...
async def test_manage_comment_invalid_action():
    ...
    try:
        await facebook.execute_action(
            "manage_comment",
            {"page_id": "123456789", "comment_id": "123456789_111_c1", "action": "invalid_action"},
            context,
        )
        assert False, "Should have raised exception"
    except Exception as e:
...
async def test_delete_comment_success():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action(
        "delete_comment", {"page_id": "123456789", "comment_id": "123456789_111_c1"}, context
    )
    data = result.result.data
...
async def test_get_insights_page():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action("get_insights", {"target_type": "page", "target_id": "123456789"}, context)
    data = result.result.data
...
async def test_get_insights_post():
    ...
    context = MockExecutionContext(responses)
    result = await facebook.execute_action(
        "get_insights", {"target_type": "post", "target_id": "123456789_111"}, context
    )
    data = result.result.data
...
async def test_get_insights_custom_metrics():
    ...
    result = await facebook.execute_action(
        "get_insights",
        {"target_type": "page", "target_id": "123456789", "metrics": ["page_views_total", "page_fan_adds"]},
        context,
    )
    data = result.result.data
