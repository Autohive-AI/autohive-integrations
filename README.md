...
[meetup](meetup): Meetup integration for managing events across multiple groups via the Meetup GraphQL API. Supports listing organized groups, full event lifecycle management (create, update, publish, delete), and cross-group event publishing. Designed for organizers managing multiple communities who want to create an event once and publish it across all their groups. Works with standard Meetup OAuth2 (no Pro account required). Includes 9 actions covering user profile, group management, and complete event workflows.
