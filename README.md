...
[hubspot](hubspot): Comprehensive HubSpot CRM integration for managing contacts, companies, deals, support tickets, tasks, marketing emails, and campaigns through HubSpot's CRM v3, v4 Associations, Marketing v3, and Conversations APIs. Supports complete contact management with email history, company CRUD with property discovery, deal pipeline management with UTC date handling and fetch_all pagination, support ticket management with conversation threading, note management (create, update, delete with multi-object associations), task management (`create_task`, `get_task`, `list_tasks`, `update_task`, `delete_task`), lists/segments with member exports, v4 Associations API for contacts, companies, and deals, call and meeting transcript retrieval (including Google Meet via the Calling Transcripts API), marketing email listing with statistics, and campaign browsing with performance analytics. Features OAuth 2.0 platform authentication with 16 HubSpot scopes, intelligent pagination, rate limiting for high-volume operations, and comprehensive search and filtering. Includes 53 actions across contacts, companies, deals, tickets, notes, tasks, lists, associations, marketing, calls/meetings, properties, and owners. Ideal for CRM automation, sales pipeline analytics, customer support workflows, and marketing campaign monitoring.
...
[github](github): Comprehensive GitHub integration for complete repository management via GitHub REST API v2022-11-28. Supports repository CRUD (create, get, list, update, delete, fork), git tree browsing and collaborator listing, branch management (list, get, create, delete, branch protection rules, compare branches), commit history and diffs, issue tracking (list, get, create, update with labels and assignees, comments, label CRUD, sub-issue hierarchies, organization issue types and fields), pull request workflows (list, get, create, update, merge, reviewer management, diffs, changed files, commits, combined status, and the full review lifecycle including pending reviews and inline comment replies), search across code, commits, issues, pull requests, repositories, users and organizations, GitHub Actions (list workflows and runs, dispatch, re-run, cancel, inspect jobs, read job logs, list artifacts, run timing), security alerts (code scanning, Dependabot, secret scanning, code quality findings, and security advisories), file operations (get, create, update, delete, plus atomic multi-file commits via push_files), webhooks (create, list, delete), gists (create, get, list, update), user, organization and team management, releases (list, get, get latest, get by tag), and tags. Features OAuth 2.0 platform authentication with repo, read:org, gist, admin:repo_hook, workflow, and security_events scopes, automatic pagination, and comprehensive error handling. Secret scanning actions never return the leaked credential. Includes 111 actions across repositories, files, branches, commits, issues, labels, sub-issues, pull requests, reviews, search, workflows, security, releases, users, organizations, teams, gists, and webhooks. Ideal for DevOps automation, code review workflows, CI/CD integration, security triage, and repository management.
