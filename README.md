...
[shotstack](shotstack): Video editing and rendering integration with the Shotstack API for creating professional videos programmatically. Supports uploading media files to Shotstack Ingest with status polling, submitting render jobs and checking their status asynchronously, composing videos from multiple clips with transitions and effects, full timeline-based editing with tracks, layers, video/image/title/audio/HTML assets, text and logo overlays with position and opacity controls, adding background audio tracks with fade and volume controls, trimming video segments, concatenating multiple videos, and auto-generated or manual caption/subtitle rendering. Features custom API key authentication with stage (watermarked sandbox) and v1 (production) environment selection, asynchronous render polling, and base64 file download for rendered output. Includes 15 actions covering file ingestion, render submission, status tracking, compositing, overlays, audio, trimming, concatenation, and captioning. Ideal for automated video production, social media content generation, and programmatic video editing workflows.
