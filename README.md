...
[microsoft-powerpoint](microsoft-powerpoint): Comprehensive presentation management integration with Microsoft PowerPoint via Microsoft Graph API for reading, creating, and manipulating presentations stored in OneDrive for Business or SharePoint. Supports presentation discovery (list with filtering by name and folder), metadata retrieval, presentation creation (blank or from template), and complete slide lifecycle management (add, update, delete slides with titles, content, and speaker notes). Features PDF export with customizable output location, presentation thumbnail access, and slide layout support (blank, title, titleContent, twoContent, comparison, titleOnly). Uses python-pptx library for slide manipulation with download-modify-upload pattern. Includes path traversal protection, file size validation (50MB download limit, 4MB upload limit for simple uploads), position validation for slide insertion, and comprehensive error handling. Features OAuth2 authentication with Files.ReadWrite and User.Read scopes. Comprises 10 actions covering presentation management, slide operations, and PDF export. Ideal for presentation automation, report generation, slide deck management, and document workflow integration with Microsoft 365.
