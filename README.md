...
[workflowmax](workflowmax): WorkflowMax (by Xero) is a cloud-based job management platform for professional services businesses including agencies, consultancies, and trades. The integration provides 28 actions covering clients and contacts, jobs, timesheets, invoices, quotes, tasks, staff, leads, costs, and purchase orders. Supports platform-based OAuth 2.0 authentication via the Xero/WorkflowMax identity provider with automatic token refresh. Ideal for automating project billing workflows, time tracking and reporting, client onboarding, sales pipeline management, and end-to-end job financial management.
