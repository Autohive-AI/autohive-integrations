...
[nz-legislation](nz-legislation): Read-only access to the Parliamentary Counsel Office's New Zealand Legislation Data API. Supports title and full-content search with legislation-specific filters, paginating through a work's complete version history, retrieving canonical version metadata and HTML/PDF/XML format links, searching official XML provisions, and reading bounded chunks of raw XML when exact markup or non-provision content is required. Uses a per-user or organisation API key in the `X-Api-Key` header for both API and XML requests, exposes daily quota state, and handles the separate five-minute IP burst limit. Includes 5 actions. Ideal for legal research, legislative monitoring, historical-version analysis, and workflows that need structured New Zealand law source data. API content is not legal advice.
