...
def handle_api_error(error: Exception) -> Dict[str, Any]:
    ...
    if isinstance(error, HttpError):
        return {"result": False, "error": f"Google Chat API error: {str(error)}"}
...
def format_space(space: Dict[str, Any]) -> Dict[str, Any]:
    ...
    formatted = {"name": space.get("name", ""), "space_type": space.get("spaceType", "UNKNOWN")}
...
def format_member(membership: Dict[str, Any]) -> Dict[str, Any]:
    ...
    formatted = {"name": membership.get("name", ""), "state": membership.get("state", "UNKNOWN")}
...
def format_reaction(reaction: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"name": reaction.get("name", ""), "user": reaction.get("user", {}), "emoji": reaction.get("emoji", {})}
...
class ListSpaces(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            result = {"spaces": spaces, "result": True}
...
class CreateSpace(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            space_body = {"displayName": inputs["display_name"], "spaceType": inputs.get("space_type", "SPACE")}
...
class SendMessage(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            message_body = {"text": inputs["text"]}

            params = {"parent": space_name, "body": message_body}
...
class ListMessages(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            result = {"messages": messages, "result": True}
...
class UpdateMessage(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            params = {"name": message_name, "body": message_body, "updateMask": inputs.get("update_mask", "text")}
...
class ListMembers(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            result = {"members": members, "result": True}
...
class AddReaction(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            reaction_body = {"emoji": inputs["emoji"]}
...
class ListReactions(ActionHandler):
    async def execute(self, inputs: Dict[str, Any], context: ExecutionContext):
        try:
            ...
            result = {"reactions": reactions, "result": True}
