...
SITE_URL = (
    "https://your-site.com"  # Replace with actual site URL (e.g., "https://example.com" or "sc-domain:example.com")
)
    ...
    try:
        ...
        async with ExecutionContext(auth=auth) as context:
            ...
            if result.get("result"):
                print("   ✓ Sites retrieved successfully")
...
async def test_query_analytics_with_filters():
    ...
    try:
        ...
        async with ExecutionContext(auth=auth) as context:
            ...
            if result.get("result"):
                print("   ✓ Filtered analytics data retrieved successfully")
                print(f"   Rows returned: {result.get('row_count')}")
                if result.get("rows"):
                    print(f"   Sample row: {result.get('rows')[0]}")
            else:
...
async def test_query_analytics_by_device():
    ...
    try:
        ...
        async with ExecutionContext(auth=auth) as context:
            ...
            if result.get("result"):
                print("   ✓ Device analytics data retrieved successfully")
                print(f"   Rows returned: {result.get('row_count')}")
                for row in result.get("rows", []):
                    print(
                        f"   - Device: {row.get('device')}, Clicks: {row.get('clicks')}, Impressions: {row.get('impressions')}"
                    )
            else:
...
async def test_list_sitemaps():
    ...
    try:
        ...
        async with ExecutionContext(auth=auth) as context:
            ...
            if result.get("result"):
                print("   ✓ Sitemaps retrieved successfully")
...
async def test_inspect_url():
    ...
    try:
        ...
        inputs = {"site_url": SITE_URL, "inspection_url": inspection_url}

        async with ExecutionContext(auth=auth) as context:
            result = await google_search_console.execute_action("inspect_url", inputs, context)

            if result.get("result"):
                print("   ✓ URL inspected successfully")
