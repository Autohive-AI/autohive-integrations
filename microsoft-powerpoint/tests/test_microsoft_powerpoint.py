...
# Create a proper ActionResult mock class
class MockActionResult:
    def __init__(self, data=None, cost_usd=0.0):
        self.data = data or {}
        self.cost_usd = cost_usd

mock_sdk.ActionResult = MockActionResult
...
class TestListPresentationsAction:
    ...
    async def test_list_presentations_success(self, mock_context):
        ...
        assert len(result.data['presentations']) == 1
        assert result.data['presentations'][0]['id'] == 'item-123'
        assert result.data['presentations'][0]['name'] == 'Quarterly Report.pptx'
...
class TestGetPresentationAction:
    ...
    async def test_get_presentation_success(self, mock_context):
        ...
        assert result.data['result'] is True
        assert result.data['id'] == 'item-123'
        assert result.data['name'] == 'Quarterly Report.pptx'
        assert result.data['size'] == 2048576
...
class TestGetSlidesAction:
    ...
    async def test_get_slides_success(self, mock_download, mock_context):
        ...
        assert result.data['slides'][0]['index'] == 1
...
class TestDeleteSlideAction:
    ...
    async def test_delete_last_slide_error(self, mock_download, mock_context):
        ...
        assert result.data['result'] is False
        assert 'Cannot delete the last slide' in result.data['error']
...
class TestExportPdfAction:
    ...
    async def test_export_pdf_success(self, mock_context):
        ...
        assert result.data['result'] is True
        assert result.data['pdf_id'] == 'pdf-123'
        assert result.data['pdf_name'] == 'Report.pdf'
...
class TestGetSlideImageAction:
    ...
    async def test_get_slide_image_success(self, mock_context):
        ...
        assert result.data['result'] is True
        assert result.data['image_url'] == 'https://image.com/slide1.png'
...
class TestCreatePresentationAction:
    ...
    @pytest.mark.asyncio
    async def test_create_presentation_path_traversal_blocked(self, mock_context):
        from microsoft_powerpoint import CreatePresentationAction
        ...
        assert result.data['result'] is False
        assert 'path traversal' in result.data['error'].lower()

    @pytest.mark.asyncio
    async def test_create_presentation_folder_path_traversal_blocked(self, mock_context):
        from microsoft_powerpoint import CreatePresentationAction

        action = CreatePresentationAction()
        result = await action.execute({
            'name': 'Valid Name',
            'folder_path': '../../../sensitive'
        }, mock_context)
...
class TestHelperFunctions:
    ...
    def test_validate_path(self):
        from microsoft_powerpoint import validate_path

        # Valid paths
        assert validate_path("") is True
        assert validate_path("Documents") is True
        assert validate_path("Documents/Reports") is True
        assert validate_path("/Documents/Reports") is True

        # Invalid paths (path traversal)
        assert validate_path("..") is False
        assert validate_path("../secret") is False
        assert validate_path("Documents/../secret") is False
        assert validate_path("/..") is False


class TestAddSlideAction:
    @pytest.mark.asyncio
    async def test_add_slide_invalid_position_zero(self, mock_context):
        from microsoft_powerpoint import AddSlideAction
        ...
        assert result.data['result'] is False
        assert 'Position must be 1 or greater' in result.data['error']

    @pytest.mark.asyncio
    async def test_add_slide_invalid_position_negative(self, mock_context):
        from microsoft_powerpoint import AddSlideAction
    ...
    @patch('microsoft_powerpoint.overwrite_file_content')
    @patch('microsoft_powerpoint.download_file_content')
    async def test_add_slide_position_out_of_range(self, mock_download, mock_overwrite, mock_context):
        from microsoft_powerpoint import AddSlideAction
        from pptx import Presentation
        ...
        assert result.data['result'] is False
        assert 'Position 10 is invalid' in result.data['error']
