...
async def overwrite_file_content(context: ExecutionContext, item_id: str, content: bytes) -> Dict[str, Any]:
    ...
    if len(content) > MAX_SIMPLE_UPLOAD_SIZE:
        raise ValueError(
            f"File size ({len(content)} bytes) exceeds simple upload limit ({MAX_SIMPLE_UPLOAD_SIZE} bytes). Large file upload not yet supported."
        )
        ...
        try:
            ...
            if slide_index > len(prs.slides):
                return ActionResult(
                    data={
                        "result": False,
                        "error": f"Slide index {slide_index} is out of range. Presentation has {len(prs.slides)} slides.",
                    },
                    cost_usd=0.0,
                )
